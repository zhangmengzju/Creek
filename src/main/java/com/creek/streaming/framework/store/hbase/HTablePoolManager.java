...
public class HTablePoolManager {
    ...
    private static final Logger                       LOGGER                = LoggerFactory
                                                                                    .getLogger(HTablePoolManager.class);

    private HTablePool                                pool                  = null;                                     //HTablePool，主要负责管理HTable实例，是线程安全的

    private static HashMap<String, HTablePoolManager> confFilePathNameToPoolManager = null;

    //线程安全的懒汉式单例
    public static synchronized HTablePoolManager getInstance(String confFilePath) throws IOException {
        if (confFilePathNameToPoolManager == null) {
            confFilePathNameToPoolManager = new HashMap<String, HTablePoolManager>();
        }
        if (confFilePathNameToPoolManager.get(confFilePath) == null) {
            confFilePathNameToPoolManager.put(confFilePath, new HTablePoolManager(confFilePath));
        }
        return confFilePathNameToPoolManager.get(confFilePath);
    }

    public HTablePoolManager(String confFilePath) throws IOException {
        LOGGER.error("[confFilePath]" + confFilePath);
        
        Configuration conf = null;
        Properties properties = new Properties();
        properties.load(HTablePoolManager.class.getResourceAsStream(
                    //"/properties/effect/"+ confFileName + ".properties"
                    confFilePath));
        
        if (confFilePath.contains("diamond")) {
