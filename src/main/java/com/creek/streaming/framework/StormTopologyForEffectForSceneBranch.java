...
import com.creek.streaming.framework.producer.effect.sceneBranch.BoltForEffect;
import com.creek.streaming.framework.source.SpoutInstance;
import com.creek.streaming.framework.utils.StreamFileUtils;

public class StormTopologyForEffectForSceneBranch {
    private static final Logger           LOGGER   = LoggerFactory.getLogger(StormTopologyForEffectForSceneBranch.class);
    ...
    public static TopologyBuilder initTopology(String topoConfPath) throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException {
        try {
            LOGGER.warn("[topoConfPath][" + topoConfPath + "]");
            topoProp.load(StormTopologyForEffectForSceneBranch.class.getResourceAsStream(topoConfPath));
        } catch (IOException e) {
        ...
        for (int i = 1; i <= spoutInstanceNum; i++) {
            ...
            if (type.equals("tt")) {
                ...
                //TT -> Spout
                File tempFile = StreamFileUtils.stream2file(StormTopologyForEffectForSceneBranch.class
        ...
        ///////////////////////////////////////////////
        //Topology setBolt 
        BoltDeclarer bd = builder.setBolt("processor", new BoltForEffect(), boltNum);
    ...
    public static void submitTopology(TopologyBuilder builder, String boltConfPath) throws AlreadyAliveException,
        ...
        ////conf submit后, bolt和spout中可以在各自的prepare()中访问到
        conf.put("boltConfPath", boltConfPath);

        StormSubmitter.submitTopology(topoProp.getProperty("topology.name"),
                conf, builder.createTopology());
        LOGGER.warn("storm cluster will start");
    }

    public static void main(String[] args) throws IOException, AlreadyAliveException,
            InvalidTopologyException, ClassNotFoundException, InstantiationException, IllegalAccessException {          
        if (args.length < 3) {
        ...
        String topoConfPath = String.valueOf(args[1]);//"/properties/effect/sceneBranch/topology.properties"
        String boltConfPath = String.valueOf(args[2]);//"/properties/effect/sceneBranch/bolt.properties"
