...
public class BoltForEffect implements IRichBolt {
    ...
    public void prepare(@SuppressWarnings("rawtypes") Map conf, TopologyContext context,
        ...
        try {
            ...
            LOGGER.error("[boltConfPath]" + boltConfPath);
            boltProp.load(BoltForEffect.class.getResourceAsStream(boltConfPath));
            for (Entry<Object, Object> ent : boltProp.entrySet()) {
                String key = (String) ent.getKey();
                String value = (String)boltProp.getProperty(key);
                LOGGER.error("[boltProp][key:" + key + "][value:" + value + "]");
    ...
    public void execute(Tuple tuple) {
        ...
        try { 
            ...
            LOGGER.error("[lastUpdateMySQLTime]" + lastUpdateMySQLTime);
        } catch (Throwable e) {
